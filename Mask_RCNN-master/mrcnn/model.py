...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    def custom_set_intersection(set1, set2):
        expanded_set1 = tf.expand_dims(set1, 1)
        expanded_set2 = tf.expand_dims(set2, 0)
        intersection = tf.reduce_any(tf.equal(expanded_set1, expanded_set2), axis=1)
        return tf.boolean_mask(set1, intersection)
    ...
    # Apply bounding box deltas and clip to image window
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        ...
        # Use the custom set intersection function
        keep = custom_set_intersection(keep, conf_keep)
    ...
    # Map over class IDs and flatten the results
    nms_keep = tf.map_fn(nms_keep_map, unique_pre_nms_class_ids, dtype=tf.int64)
    nms_keep = tf.reshape(nms_keep, [-1])
    nms_keep = tf.boolean_mask(nms_keep, nms_keep > -1)

    # Custom set intersection between `keep` and `nms_keep`
    keep = custom_set_intersection(keep, nms_keep)
